 ...
 * Updated for Launchpad dual-color theme.
 */
export const LimelightNav = ({
  ...
  return (
    <nav
      className={`relative inline-flex items-center h-16 rounded-lg px-2 ${className}`}
      style={{
    ...
    >
      ...
      {/* Limelight indicator with Launchpad gradient */}
      <div
        ref={limelightRef}
        className={`absolute top-0 z-10 w-11 h-[5px] rounded-full ${isReady ? 'transition-[left] duration-400 ease-in-out' : ''
          } ${limelightClassName}`}
        style={{
          left: '-999px',
          background: 'var(--lp-gradient-primary)',
          boxShadow: '0 15px 20px rgba(0, 169, 255, 0.25)'
        }}
      >
        {/* Glow cone effect */}
        <div
          ...
          style={{
            background: 'linear-gradient(to bottom, rgba(0, 169, 255, 0.3), transparent)'
          }}
        />
      </div>
    </nav>
